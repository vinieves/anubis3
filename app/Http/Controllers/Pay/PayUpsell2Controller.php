...
class PayUpsell2Controller extends Controller
{
    ...
    public function processUpsell2(Request $request)
    {
        ...
        $oferta = session('pay_oferta_atual');
        $upsell2Id = $oferta['upsell2'] ?? env('CHECKOUT_ID3');
        
        // Usar o upsell específico da oferta
        $upsell2Service = new Upsell2CartPandaService($upsell2Id);
        
        logger()->info('[PAY] Processando upsell2', [
            'oferta_id' => $oferta['id'] ?? 'desconhecida',
            'upsell2_id' => $upsell2Id
        ]);

        // Processa o upsell2
        $result = $upsell2Service->processUpsell2($previousOrderData);

        // Sempre redireciona para thankyou do pay
        return response()->json([
            'success' => true,
            'redirect_url' => '/pay/thankyou'
        ]);
    }
}
